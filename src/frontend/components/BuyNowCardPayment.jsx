...
import { ProductGreaterThanZero } from '../../actions/Products';
import { createSingleProductCardPayment } from '../../actions/VariableChangePage';
import { decryptUserDataLocalStorage, encryptSingleData, decryptSingleData } from '../../utils/helper';
const BuyNowCardPayment = () => {
  const dispatch = useDispatch();
  const history = useNavigate();
  const urlParam = useLocation();
  const uData = JSON.parse(decryptUserDataLocalStorage());
  const { productData, isLoading } = useSelector((state) => state.productsApi);
  ...
  const [overAllPPrice, setOverAllPPrice] = useState(0.00);
  const [qtys, setQtys] = useState('1');

  const arr = [1, 2, 3, 4, 5];

  useEffect(() => {
    if (new URLSearchParams(urlParam.search).get('id')) {
      setProductId(decryptSingleData(new URLSearchParams(urlParam.search).get('id')))
      dispatch(ProductGreaterThanZero(decryptSingleData(new URLSearchParams(urlParam.search).get('id'))))
    }
  }, [])

  useEffect(() => {
    let sum = productData?.price;
    ...
    setOverAllPPrice(productData?.price)
  }, [productData])
  ...
  const updateCartQty = (e) => {
    if (e?.target?.value) {
      ...
      var tempSum = productData?.price * e?.target?.value
      setSubTotal(tempSum)
      setTaxes('8.50')
      setDeliChar('50')
      setTotalPrice(Math.round(parseFloat(tempSum) + parseFloat(taxes) + parseFloat(deliChar)))
      setOverAllPPrice(tempSum)
    }
  }
  ...
  const handlePaymentForm = useFormik({
    ...
    onSubmit: values => {
      ...
      dispatch(createSingleProductCardPayment(formData, history));
    },
  });
      ...
      <div className="h-screen grid grid-cols-3 px-5 py-5">
        <div className="lg:col-span-2 col-span-3 bg-indigo-50 space-y-8 px-12">
          <form id="payment-form" onSubmit={handlePaymentForm.handleSubmit}>
            ...
            {
              productId ? <button type="submit" className="submit-button px-4 py-3 rounded-full bg-blue-400 text-white focus:ring focus:outline-none w-full text-xl font-semibold transition-colors" disabled={enableDisable}>
                {(() => {
              ...
              (isLoading == true) ?
                ...
                <li className="grid grid-cols-6 gap-2 border-b-1">
                  <div className="col-span-1 self-center">
                    <img src={productData?.p_image} alt={productData?.p_name} className="rounded w-full" />
                  </div>
                  <div className="flex flex-col col-span-2 pt-2">
                    <span className="text-gray-600 text-md font-semi-bold"><Link to={`/product/${encryptSingleData(productData?.id)}`}>{productData?.p_name}</Link></span>
                    {/* <span className="text-gray-400 text-sm inline-block pt-2">Red Headphone</span> */}
                  </div>
                  <div className="col-span-3 pt-3">
                    <div className="flex items-center space-x-2 text-sm justify-between">
                      <span className="text-gray-400">{productData?.price} &#8377; </span>
                      ...
                      <span className="text-pink-400 font-semibold inline-block">&#8377; {overAllPPrice}</span>
                    </div>
                  </div>
                </li>
...
export default BuyNowCardPayment